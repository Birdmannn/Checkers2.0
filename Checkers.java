...
public class Checkers extends Application {
    ...
    private Button saveGameButton;  //New button added for saving the game into an xml file (Given as exercise)
    private Button loadGameButton;  //New button added for loading game from xml file
    ...
    public void start(Stage stage) {
        ...
        loadGameButton.setOnAction( e -> board.loadGame() );

        board.setOnMousePressed( e -> board.mousePressed(e) );

        /* Set the location of each child by calling its relocate() method */

        /* Relocated and reconstructed all the buttons*/                                 //here
        board.relocate(20,20);
        newGameButton.relocate(370, 80);
        resignButton.relocate(370, 245);
        saveGameButton.relocate(370, 190);
        loadGameButton.relocate(370, 135);
        message.relocate(20, 370);

        loadGameButton.setFont(Font.font(null,FontWeight.BOLD,12));
        newGameButton.setFont(Font.font(null,FontWeight.BOLD,12));
        saveGameButton.setFont(Font.font(null,FontWeight.BOLD,12));
        resignButton.setFont(Font.font(null,FontWeight.BOLD,12));
        resignButton.setStyle("-fx-text-fill: red");
        ...
        root.getChildren().addAll(board, newGameButton, resignButton, saveGameButton, loadGameButton, message);
    ...
    private class CheckersBoard extends Canvas {
        ...
        //-----------------------newly added static variables for FileIO------------------------------
        private static String DATA_FILE_NAME = ".checkers_data";
        private static File userHomeDirectory = new File(System.getProperty("user.home"));
        private static File dataFile = new File(userHomeDirectory,DATA_FILE_NAME);
        //--------------------------------------------------------------------------------------------
        ...
        void loadGame() {
            if(! dataFile.exists()) {
                message.setText("No saved file found.");
                return;
            }

            currentPlayer = board.setBoard(dataFile);    //couldn't decipher their hint, so I wrote a new setBoard method in the CheckersData class.
        ...
        void saveGame() {                               //The exercise says to use any text file to save the game, so I used XML; couldn't make one
            boolean saved = false;                      //to save myself the energy from figuring how to parse my text file.
            PrintWriter out;

            try {
                out = new PrintWriter(dataFile);
                out.println("<?xml version=\"1.0\"?>");
                out.println("<checkers_data>");
                for(int i = 0; i < 8; i++) {
                    out.print("   <board");
                    for(int j = 0; j < 8; j++) {
                        out.print(" point" + i + j + "='" + board.pieceAt(i,j) + "'");
                    }
                    out.println("/>");
                }
                out.println("   <player p='" + currentPlayer + "'/>");
                out.println("</checkers_data>");
                ...
                message.setText("Error encountered while saving game.");
            }
            finally {
                if(saved)
    ...
    private static class CheckersData {
        ...
        /*Wrote this method to read the data file and return currentplayer*/
        public int setBoard(File dataFile) {
            DocumentBuilder docReader;
            Document xmlDoc;
            Element element;
            int[][] newBoard = new int[8][8];

            int newCurrentPlayer = EMPTY;
            try {
                ...
                for(int i = 0; i < children.getLength(); i++) {
                    ...
                    if(child instanceof Element pointElement) {
                        //load all points back into the array
                        if(pointElement.getTagName().equals("player"))
                            newCurrentPlayer = Integer.parseInt(pointElement.getAttribute("p"));
                        else if(pointElement.getTagName().equals("board"))
                            for(int k = 0; k < 8; k++)
                                newBoard[j][k] = Integer.parseInt(pointElement.getAttribute("point" + j + k));
                            j++;
                    }
                }
            }
            ...
            for(int j = 0; j < 8; j++)
                for(int k = 0; k < 8; k++)
                    System.out.println("At point " + j + k + " = " + pieceAt(j,k));       // for testing.


            return newCurrentPlayer;
        }  //end of the method used.
